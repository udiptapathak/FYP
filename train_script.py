...
from quantization.threshold import threshold
...
def create_model(vocab_size, quant_type=None, quant_method=None, bit_num=None, quant_pattern=None):
    ...
    quant_method  - quant method to use, if quant_type is None, it should also be None
                    For 'quantization', should be one of ['basic']
                    For 'binarization', should be one of ['basic']
    ...
    if quant_type == None:
        ...
        print("creating baseline transformer with no quantization/binarization!")
    ...
    # here all binarization method skip final layer
        ...
        if quant_pattern == 'ALL_QK':
            binarize(model, 'ALL', skip_final=True, kv_only=True)
        else:
    ...
    elif quant_type == 'thresholding':
            threshold(model, 'ALL', skip_final=True, kv_only=True)
    return model
if __name__ == '__main__':
    ...
    parser.add_argument("--quant_type", 
                        type=str, 
                        help="whether to conduct quantization or binarization or thresholding", 
                        choices=['quantization', 'binarization','thresholding'],
                        default=None)
